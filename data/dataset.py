...
class yoloDataset(data.Dataset):
    image_size = 448
    def __init__(self, root, list_file, is_train, transform=[], debug=False):
        print('data init')
        self.root=root
        self.is_train = is_train
        self.transform = [transforms.ToTensor()] + transform
        self.debug = debug
        self.fnames = []
        self.boxes = []
        self.labels = []
    ...
    def encoder(self,boxes,labels):
        ...
        for i in range(cxcy.size()[0]):
            ...
            # assign box target prob and class target prob
            target[int(ij[1]),int(ij[0]),4] = 1
            target[int(ij[1]),int(ij[0]),9] = 1
            target[int(ij[1]),int(ij[0]),int(labels[i])+9] = 1

            # assign box target cxcy and target wh
...
def main():
    ...
    train_dataset = yoloDataset(root=file_root,list_file='voc12_trainval.txt',is_train=True,transform = [transforms.ToTensor()] )
